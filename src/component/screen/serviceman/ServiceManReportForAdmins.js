...
export default function ServicemanReport({navigation, route}) {
  ...
  const handleDone = () => {
    ...
    fetchServicesByServicemanReport();
  };
...
const handleErrors = (txt, attr) => {
  ...
  const fetchServicesByServicemanReport = async (item = '') => {
    ...
    var result = await getData(
      'services/byServicemanReport/' + route.params.id + str,
    );
    var results = await getData(
      'services/byServicemanCompleteReport/' + route.params.id + str,
    );
    if(results.status){
  ...
  const FilterDates = ({item}) => {
    return (
      <TouchableOpacity
        style={{
          width: width * 0.22,
          backgroundColor:  theme == 'light' ? COLORS.white : 'black',
          borderRadius: 8,
          borderColor: '#d0d0d0',
          borderWidth: 0.5,
          alignItems: 'center',
        ...
        onPress={() => {
          if (item.name != 'Custom') {
            fetchServicesByServicemanReport(item.name);
          } else {
  ...
  useEffect(() => {
    fetchServicesByServicemanReport('Today');
    selectedLng();
  }, [isFocused]);
  
  const handlePay=async()=>{
    var body={servicemanid:route.params.id,amount:inputs.amount,remaining:services.needtopay-inputs.amount}
      ...
      <View style={{flex: 1}}>
        ...
        <View
          ...
          <Text
            ...
            {strings.TOTAL}  &#x20b9;{services.amountcollectedfromservice||'--'} {strings.FROM} {services.totalservice||'--'} service
          </Text>
          <SafeAreaView>
            <FlatList
              horizontal={false}
