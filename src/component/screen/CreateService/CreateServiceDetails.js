...
export default function CreateServiceDetails({navigation}) {
  ...
  const fetchServiceDetails = async () => {
    setLoader(true);
    var result = await getData('services');
    if (result.status) {
      setServices(result.data.reverse());
      setServiceFilter(result.data);
    }
    setLoader(false);
  };

  const fetchViewer = async serviceid => {
    var result = await getData('services/serviceseenreport/' + serviceid);
    if (result.status) {
      setSeen(result.data);
    }
  };
  ...
  const handleFilter = txt => {
    setFilterInput(txt);
    const data = serviceFilter.filter(
      item =>
      ...
      <View style={{alignItems: 'center'}}>
        <View
          ...
          <View
            ...
            <View style={{marginTop: 4, marginLeft: 10, width: width * 0.8}}>
              <Text
                numberOfLines={1}
              ...
              <TouchableOpacity
                onPress={() => {
                  refRBSheet.current.open(), fetchViewer(item.service_id);
                }}>
                <View
                  style={{
                    flexDirection: 'row',
                  ...
                  <MCI
                    name="eye"
                    size={16}
                    style={{color: COLORS.red, padding: 1}}
                  />
            ...
            <TouchableOpacity
              onPress={() =>
                navigation.navigate(
                  'Edit Create Service',
                  {id: item.service_id},
                  {typesofserviceid: item.typesofservices_id},
                )
              }
              style={{
                width: 30,
      ...
      <View style={{flex: 1}}>
        <View style={{alignItems: 'center'}}>
          <SearchInput
            ...
            onChangeText={txt => handleFilter(txt)}
            height={40}
        ...
        <View
          ...
          <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
            <Text
              ...
              {strings.SEEN_BY} {services.totalviews}
            </Text>
          ...
          />
          <View style={{borderWidth: 0.5, borderColor: '#d0d0d0'}} />
          {seen.map(item => {
            return (
              <>
                <ScrollView>
