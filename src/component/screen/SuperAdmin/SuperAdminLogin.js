...
export default function SuperAdminLogin({navigation}) {
  ...
  const handleLogin = async () => {
    if (validate()) {
      var result = await postData('superadmin/login', {
        username: inputs.username,
        password: inputs.password,
      });
      if (result.status) {
        storeData('SUPERADMIN', result.data);
        navigation.replace('SuperAdminMainPage');
      } else {
  ...
  useEffect(function () {
    const fetchData = async () => {
      const data = await getStoreData('SUPERADMIN');
      if (data) {
        navigation.replace('SuperAdminMainPage');
      }
    };
