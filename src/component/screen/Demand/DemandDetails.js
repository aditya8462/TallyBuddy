...
import {deleteData, getData} from '../../Connection/FetchServices';
...
export default function DemandDetails({navigation}) {
  ...
  const fetchDemand = async () => {
    setLoader(true);
    var result = await getData('demand');
    if (result.status) {
      setDemand(result.data);
      setDemandFilter(result.data);
    }
    setLoader(false);
  };
  ...
  const handleFilter = txt => {
    setFilterInput(txt);
    const data = demandFilter.filter(
      item =>
  ...
  const handleDelete = async (item) => {
    const result = await deleteData('demand/' + item.demand_id);
    if (result.status) {
