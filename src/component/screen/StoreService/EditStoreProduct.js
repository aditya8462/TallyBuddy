...
export default function EditStoreProduct({navigation, route}) {
  ...
  const validate = () => {
    ...
    if (!brandlist) {
      handleErrors('Please Input Brand Name', 'brand_id');
      isValid = false;
    }
    if (!modellist) {
      handleErrors('Please Input Model', 'model_id');
      isValid = false;
    }
    if (!vendorlist) {
      handleErrors('Please Input Vendor', 'vendor_id');
      isValid = false;
    }
    if (!inputs.costprice) {
  ...
  const fetchProducts = async () => {
    ...
    // var st = await getStoreData = async ('STORE');
